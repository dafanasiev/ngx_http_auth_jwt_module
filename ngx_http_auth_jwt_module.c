...
static ngx_http_auth_jwt_kvp_t ngx_methods_names[] = {
...
static ngx_int_t ngx_http_auth_jwt_handler(ngx_http_request_t *r) {
    ...
    // Validate the jwt
    //If no signature is used (JWS, alg="none") or key is NULL, then no validation is done other than formatting.
...
static ngx_int_t ngx_http_auth_jwt_header_var(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    const char *header = (char *) auth_jwt_safe_string(r->pool, name->data + sizeof("jwt_header_") - 1,
                                                       name->len - (sizeof("jwt_header_") - 1));
...
static ngx_int_t ngx_http_auth_jwt_grant_var(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    ngx_str_t *name = (ngx_str_t *) data;
    const char *grant = (char *) auth_jwt_safe_string(r->pool, name->data + sizeof("jwt_grant_") - 1,
                                                      name->len - (sizeof("jwt_grant_") - 1));
